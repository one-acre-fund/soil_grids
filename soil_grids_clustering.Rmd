...
runKMeansRaster <- function(dat, numClusters=2:8) {
  ...
  kClusts <- lapply(numClusters, FUN=function(k) {
    ...
    kmClustRes[!idx] <- NA
    kmClustRes[idx] <- clusterRes$cluster
...
plot(numClusters, lapply(soilClusters, FUN=function(x) { sum(x[[2]]$withinss) }),   
...
# bind all the scenario data into a single df
scenarioDat <- do.call(rbind, lapply(soilClusters, function(clusterDat){
  clusterDat[[2]]$totalCluster  = max(clusterDat[[2]]$cluster)
  df <- cbind(cluster = clusterDat[[2]]$cluster, totalCluster = clusterDat[[2]]$totalCluster, soilDf)
  return(df)
