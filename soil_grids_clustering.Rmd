...
```{r, scrap}
rasterList <- importRasterAsList(tifFinder("af_PHIHOX_T__M"))

test <- Map("*", rasterList, weightList)

secondTest <- stack(test)

thirdTest <- mean(secondTest)
```
...
cropRaster <- function(rasterObj, borders){
  cropped = crop(rasterObj, borders)
}



test <- crop(testList[[1]], keBoundaries)
plot(test)

maskTest <- extract(testList[[1]], keBoundaries)
plot(maskTest)

testList[[1]][keBoundaries,]

# rasterList <- importRasterAsList(testInput)
# 
