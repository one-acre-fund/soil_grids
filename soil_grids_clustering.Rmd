...
```{r}
soilDf <- raster::as.data.frame(soilStack, xy = TRUE)
```
...
runKMeans <- function(dat, datCols, completeVar, numClusters=2:8) {
  ...
  # output - k means cluster assignments in list form with the NAs in place so that I can convert the data back to raster
  
  kClusts <- lapply(numClusters, FUN=function(k) {
    datA = dat[,c(datCols)]
    
    matNa <- as.data.frame(is.na(datA))
    matNa$isComplete <- complete.cases(matNa)
    ...
    scaled <- scale(datA)
    set.seed(2017201616)
    kmeanRes = kmeans(scaled, centers=k, nstart=50, iter.max=100)
    
    # and then return a similarly sized matrix / df
    matNa[!is.na(matNa)] <- kmeanRes$cluster
  })
  return(kClusts)
}
...
```{r}
soilCols <- names(soilDf)[!names(soilDf) %in% c("x", "y")]

soilClusters <- runKMeans(soilDf, c(soilCols), completeVar = "pH", numClusters)
...
Convert the cluster data to raster and add it into the `soilStack`.

```{r}
clusterRaster <- lapply(soilClusters, function(x){
  clusterVar = as.matrix(x$cluster)
  colnames(clusterVar) = NULL
  r = raster(soilStack)
  r = setValues(r, clusterVar)
  return(r)
})
```
