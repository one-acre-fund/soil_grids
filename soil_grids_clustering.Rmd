...
getKenyaValues <- function(rasterList){
  ...
  rlCropped <- lapply(rasterList, function(x){crop(x, extent(westernMap))})
  rlMask <- lapply(rlCropped, function(x){mask(x, westernMap)})
...
if(!file.exists(paste(dataDir, "output", "pH_weighted_layer.tif", sep = "/")) | forceUpdate){
  ...
  pHLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_PHIHOX_T__M"))), weightList)))
...
if(!file.exists(paste(dataDir, "output", "carbon_weighted_layer.tif", sep = "/")) | forceUpdate){
  carbonLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_ORCDRC_T__M"))), weightList)))
...
if(!file.exists(paste(dataDir, "output", "texture_weighted_layer.tif", sep = "/")) | forceUpdate){
  textureLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_TEXMHT_T__M"))), weightList)))
...
if(!file.exists(paste(dataDir, "output", "sand_weighted_layer.tif", sep = "/")) | forceUpdate){
  sandLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_SNDPPT_T__M"))), weightList)))
...
if(!file.exists(paste(dataDir, "output", "silt_weighted_layer.tif", sep = "/")) | forceUpdate){
  siltLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_SLTPPT_T__M"))), weightList)))
...
if(!file.exists(paste(dataDir, "output", "clay_weighted_layer.tif", sep = "/")) | forceUpdate){
  clayLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_CLYPPT_T__M"))), weightList)))
...
if(!file.exists(paste(dataDir, "output", "cec_weighted_layer.tif", sep = "/")) | forceUpdate){
  cecLayer <- sum(stack(addWeights(getKenyaValues(importRasterAsList(tifFinder("af_CEC_T__M"))), weightList)))
...
nitrogenLayer <- getKenyaValues(importRasterAsList(tifFinder("af250m_nutrient_n_m_agg30cm.tif")))[[1]] ## this is also in a different projection system.
```
...
```{r}
zoneTextureLayer <- getKenyaValues(importRasterAsList(tifFinder("af_agg_30cm_TEXCLSS__M_1km.tif")))[[1]]
zoneTextureLayer <- projectRaster(zoneTextureLayer, pHLayer)

zoneCoarseLayer <- getKenyaValues(importRasterAsList(tifFinder("af_agg_30cm_CRFVOL__M_1km.tif")))[[1]]
zoneCoarseLayer <- projectRaster(zoneCoarseLayer, pHLayer)

zoneLayer <- getKenyaValues(importRasterAsList(tifFinder("af_ERZD__M_1km.tif")))[[1]]
...
```{r}
soilStack <- stack(pHLayer, carbonLayer, sandLayer, clayLayer, cecLayer, zoneTextureLayer, zoneCoarseLayer, zoneLayer)
plot(soilStack)
```
...
```{r}
soilDf <- raster::as.data.frame(soilStack, xy = TRUE, na.rm = TRUE)
```
...
runKMeans <- function(dat, datCols, completeVar, numClusters=2:8) {
...
soilClusters <- runKMeans(soilDf, c(soilCols), completeVar = "pH_weighted_layer", numClusters)
...
**Interpretation**: There isn't a clear kink in the graph. Running more than 10 clusters likely exceeds what we can manage in the field anyway. It seems to be the case that we max out the marginal benefit around 6 clusters but the absence of a clear elbow in the graph indicates that we still minimize within cluster variance as we continue to add clusters greater than 6 and less than 10. 

> Six, however, seems like a reasonable inflection point for the purposes of structuring the trials.
...
```{r}
mapKClusts <- function(kClusts, dat) {
    gpsCols <- grep("x|y", names(dat))
    tmp <- lapply(lapply(kClusts, FUN=function(kClust) {
      kGps <- cbind(dat[,gpsCols], cluster=kClust$cluster)
      ggplot(kGps, aes(x=x,
             y=y,
             color=as.factor(cluster))) + 
        geom_point() + 
        labs(title = paste0("k=", max(kGps$cluster)))
    }), plot)
    return(tmp)
}

scenarioDat <- do.call(rbind, lapply(soilClusters, function(clusterDat){
  clusterDat$totalCluster  = max(clusterDat$cluster)
  df <- cbind(cluster = clusterDat$cluster, totalCluster = clusterDat$totalCluster, soilDf)
  return(df)
  
}))

mapClusters <- function(dat, nClusters){
  gph <- dat %>%
    filter(totalCluster == nClusters) %>%
    ggplot(., aes(x = x, y = y, color = as.factor(cluster))) +
    geom_point()
}
```

I'm mapping the data in `ggplot` as points but I really should be converting it back to raster to make a nicer map. It appears that there are ways to [implement k-means with raster data](https://www.r-exercises.com/2018/02/28/advanced-techniques-with-raster-data-part-1-unsupervised-classification/) but this is not familiar to me and thus hesitate to try to implement them now at the last minute.

```{r}
#mapKClusts(soilClusters, soilDf) # this was leading to strange results so I'm printing the variables in a more manual way...

for(i in 2:10){
  print(mapClusters(scenarioDat, i) + labs(title = paste0("clusters = ", i)))
}
...
ratioTab <- scenarioDat %>%
  ...
  summarize(n = n()) %>%
  mutate(freq = n / sum(n),
         freq = paste0(round(freq,2) * 100, "%"))

kable(ratioTab, format = 'markdown')
```

Then the 6 AEZ scenario:

```{r}
ratioTab %>%
  filter(totalCluster == 6) %>%
...
Or we could drop one of the smaller zones and consolidate.

Should I bring in site GPS to determine the soil profiles of each site? That seems like the most actionable version of this but that would take away the benefit of the broader coverage of raster data. The next steps as I can best project them are:

* associate sites with each of the soil profiles created in the soil clustering (the 6 cluster scenario if we agree that's the appropriate one)
* see which sites are firmly in each zone

From there I'm a bit fuzzy on what it would mean to assign trial officers. I assume that means that we'd determine the specific sites where trial officers would be working.
