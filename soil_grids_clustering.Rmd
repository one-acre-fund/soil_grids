...
if(!file.exists(paste0(dataDir, "/GADM_2.8_KEN_adm2.rds"))){
  keBoundaries <- getData("GADM", country='KE', level=1, path = dataDir) 
} else {
...
combineRasters <- function(rasterList, weightList){
  ...
  #stack = stack(rasterList)
    ...
    r = lapply(1:length(rasterList), function(x){
        calc(rasterList[[x]], function(y) y * weightList[[x]])
      })
  ...
  # } else {
  #   cat("There aren't weights for all the raster layers. Please reconcile the weights")
  # }
